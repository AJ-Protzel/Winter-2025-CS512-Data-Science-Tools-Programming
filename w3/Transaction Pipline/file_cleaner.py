...
# DESCRIPTION_MAP_PATH = Path('Configs/Maps/description_map.txt')
# CATEGORY_MAP_PATH = Path('Configs/Maps/category_map.txt')
# MERGED_FILE_PATH = Path('Data/merged.csv')
...
# def merge_files():
#     """Merge all files in the Data folder into one CSV file called merged.csv without duplicating headers."""
#     files_to_merge = [f for f in Path('Data').iterdir() if f.is_file() and f.suffix == '.csv']
...
#     """Remove all old CSV files from the Data folder after merging them."""
#     files_to_remove = [f for f in Path('Data').iterdir() if f.is_file() and f.suffix == '.csv' and f != MERGED_FILE_PATH]
    
#     for file in files_to_remove:
#         file.unlink()

# def main():
#     """Main function to load configuration, list directories, process files, merge them, and remove old files."""
...
#     # Merge all cleaned files into one CSV file called merged.csv
#     merge_files()

#     # Remove old CSV files from the Data folder
#     remove_old_files()
