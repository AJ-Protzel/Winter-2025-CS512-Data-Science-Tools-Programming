...
def create_popup(root, line, next_line_callback, cancel_callback):
    ...
    for col in HEADER:
        ...
        if col == "Date":
            ...
            if col == "Type":
                entry.insert(0, type_ if type_ else "")
            elif col == "Bank":
                entry.insert(0, bank if bank else "")
            elif col == "Card":
                entry.insert(0, card if card else "")
            entries[col] = entry
    ...
    def on_enter():
        ...
        with open(os.path.join(current_dir, 'Data', 'dirty.csv'), 'a', newline='') as file:
...
bad_lines_path = current_dir / 'Data' / 'bad_lines.txt'
